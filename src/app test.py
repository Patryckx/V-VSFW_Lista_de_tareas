...
def home():
    if 'loggedin' in session and session['loggedin']:
        ...
        if user_id:
            ...
            # Si no se pudo obtener el user_id, muestra un mensaje de error o redirige al usuario a algún lugar apropiado
            flash('No se pudo obtener el ID del usuario.')
            return redirect(url_for('login'))
    else:
        # Si no está autenticado, redirige al usuario a iniciar sesión
        return redirect(url_for('login'))
